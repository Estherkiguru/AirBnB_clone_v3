...
def get_amenity(amenity_id):
    """Retrieve amenity using to class id,
...
def delete_amenity(amenity_id):
    """Delete object Amenity if exits, else 404 error"""
    if amenity_id:
...
def response_amenity():
    """Post a request to create a new amenity"""
...
def update_amenity(amenity_id):
    """Updates the attributes from an Amenity object"""
    if amenity_id:
        amenities = storage.get(Amenity, amenity_id)
        if amenities is None:
            abort(404)
